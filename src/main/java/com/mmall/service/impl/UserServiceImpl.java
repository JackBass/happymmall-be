...
public class UserServiceImpl implements IUserService {
    ...
    //检查用户Email
    @Override
    public ServerResponse<java.lang.String> register(User user){
        ...
        if(StringUtils.isNotBlank(type)){
            ...
            if(Const.EMAIL.equals(type)){
               int countResult = userMapper.checkEmail(str);
                if(countResult > 0){
                    return ServerResponse.createByErrorMessage("E-mail已存在");
                }
            }
        }else{
    ...
    public ServerResponse<String> resetPassword(String passwordOld,String passwordNew,User user){
        //防止横向越权，需要校验一下这个用户的旧密码，一定要指定这个用户，因为我们会查询一个count(1) 如果不指定id，那么结果就是true了
        //count > 0
    ...
    public ServerResponse<User> updateInformation(User user){
        //username 是不能被更新的
        //E-mail也要进行一个校验，检验新的Email是不是已经存在，并且存在的email如果相同的话，不能是我们当前的用户的

        int countResult = userMapper.checkEmailByUserId(user.getEmail(),user.getId());
        if(countResult > 0){
            return ServerResponse.createByErrorMessage("Email已经存在，请跟换Email再尝试更新");
        }
        User updateUser = new User(); //有个疑问，这么创建对象合适吗？
